function getVideoPayload(source, embedTag) {
  if (!embedTag) {
    payload = '<div class="modal-body customModalBody videoModalBody">' +
  ...
  if (source == 'local') {
    payload = '<div class="modal-body customModalBody">' +
...
function customModal(options) {
  ...
  html = '<div id="customModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="message-modal" aria-hidden="true">';
  html += that.style;
  ...
  if (that.type == 'general') {
    html += '<div class="modal-body customModalBody">';
    if (that.loadPage) {
  ...
  html += '</div>';
  if (that.showFooter) {
    html += '<div class="modal-footer customModalFooter">';
  ...
  $("#customModal").modal();
  if (that.loadPage) {
    $("#customModal .customModalBody").load(that.loadPage);
  }
